<?php 
	...
	//Incluir la clase para las multiceldas
	include('pdf_mc_table_productos_fecha.php');
	//Incluir la libreria FPD
	include_once("fpdf184/fpdf.php");

//Se capturan las fechas mandadas por el formulario
if (isset($_POST['fecha'])) {
	$fecha_inicial= trim($_POST['fecha_inicial']);
	$fecha_final= trim($_POST['fecha_final']);

//Cuerpo de la tabla, conexion y consulta a la tabla productos
	$db = new dbConexion();
	$connString = $db->getConexion();
	$result = mysqli_query($connString, "SELECT * FROM productos WHERE ACTIVO=1 AND fecha_registro BETWEEN '{$fecha_inicial}' AND '{$fecha_final}'") or die("Error en la BD: ".mysqli_error($connString));
	$header = mysqli_query($connString, "SELECT * FROM productos WHERE ACTIVO=1 AND fecha_registro BETWEEN '{$fecha_inicial}' AND '{$fecha_final}'");
...
//Campos de la tabla para extraer datos
$pdf->SetTextColor(255,255,255);
$pdf->SetFillColor(79,78,77);
...
//Se mustran los datos extraidos de la consulta realizada
...
//output the pdf
$pdf->Output('Reporte_Productos_Fecha.pdf', 'I');
}
?>