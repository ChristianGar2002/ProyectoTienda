...
class PDF extends FPDF
{
	...
	$result = mysqli_query($connString, "SELECT * FROM formulario1") or die("Error en la BD: ".mysqli_error($connString));
	$header = mysqli_query($connString, "SELECT * FROM formulario1");
...
$pdf->Ln();
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, utf8_decode($row['nacionalidad']), 1);
		$pdf->Cell($w[2], 6, utf8_decode($row['primaria_estudio']), 1);
		$pdf->Cell($w[3], 6, $row['telefono_alumno'], 1);
		$pdf->Cell($w[4], 6, utf8_decode($row['discapacidad']), 1);
		$pdf->Ln();
	}
	$pdf->Ln();
	$pdf->Ln();
	...
	$pdf->SetFont('Arial', 'B', 12);
	$w = array(20, 45,70,70,70);
	//Declaramos los encabezados de la tabla
	$pdf->Cell(20, 12, 'FOLIO', 1);
	$pdf->Cell(45, 12, 'Promedio_primaria', 1);
	$pdf->Cell(70, 12, 'Certificado', 1);
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, $row['promedio_primaria'], 1);
		$pdf->Cell($w[2], 6, $row['certificado'], 1);
		$pdf->Cell($w[3], 6, utf8_decode($row['tienes_hermanos']), 1);
		$pdf->Ln();
	}
	$pdf->Ln();
	$pdf->Ln();
...
$pdf->Ln();
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, utf8_decode($row['domicilio']), 1);
		$pdf->Cell($w[2], 6, utf8_decode($row['colonia']), 1);
		$pdf->Cell($w[3], 6, utf8_decode($row['localidad']), 1);
		$pdf->Cell($w[4], 6, utf8_decode($row['municipio']), 1);
		$pdf->Ln();
	}
	$pdf->Ln();
	$pdf->Ln();
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, $row['codigo_postal'], 1);
		$pdf->Cell($w[2], 6, utf8_decode($row['estado']), 1);
		$pdf->Cell($w[3], 6, $row['correo'], 1);
		$pdf->Cell($w[4], 6, $row['telefono_tutor'], 1);
		$pdf->Cell($w[5], 6, $row['telefono_familiar'], 1);
		$pdf->Ln();
	}
	$pdf->Ln();
	$pdf->Ln();
	...
	//Declaramos los encabezados de la tabla
	$pdf->Cell(20, 12, 'FOLIO', 1);
	$pdf->Cell(100, 12, 'Taller_1', 1);
	$pdf->Cell(140, 12, 'Taller_2', 1);
	$pdf->Ln();
	$pdf->SetFont('Arial', '', 12);
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, utf8_decode($row['carrera_1']), 1);
		$pdf->Cell($w[2], 6, utf8_decode($row['carrera_2']), 1);
		$pdf->Ln();
	}
	$pdf->Ln();
	$pdf->Ln();
	...
	//Declaramos los encabezados de la tabla
	$pdf->Cell(20, 12, 'FOLIO', 1);
	$pdf->Cell(100, 12, 'Taller_3', 1);
	$pdf->Cell(140, 12, 'Taller_4', 1);
	$pdf->Ln();
	$pdf->SetFont('Arial', '', 12);
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, utf8_decode($row['carrera_3']), 1);
		$pdf->Cell($w[2], 6, utf8_decode($row['carrera_4']), 1);
		$pdf->Ln();
	}
	$pdf->Ln();
	$pdf->Ln();
	...
	{
		$pdf->Cell($w[0], 6, $row['FOLIO'], 1);
		$pdf->Cell($w[1], 6, $row['fecha_registro'], 1);
		$pdf->Ln();
	}
