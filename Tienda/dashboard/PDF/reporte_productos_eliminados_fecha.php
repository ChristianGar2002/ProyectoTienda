<?php 
	...
	include('pdf_mc_table_productos_eliminados_fecha.php');
...
//Cuerpo de la tabla, conexion y consulta a la tabla productos
	...
	$result = mysqli_query($connString, "SELECT * FROM productos WHERE ACTIVO=2 AND fecha_eliminacion BETWEEN '{$fecha_inicial}' AND '{$fecha_final}'") or die("Error en la BD: ".mysqli_error($connString));
	$header = mysqli_query($connString, "SELECT * FROM productos WHERE ACTIVO=2 AND fecha_eliminacion BETWEEN '{$fecha_inicial}' AND '{$fecha_final}'");
...
//output the pdf
$pdf->Output('Reporte_Productos_Eliminados_Fecha.pdf', 'I');
}
?>